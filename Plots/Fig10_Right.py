...
data_complete = np.genfromtxt("data/Fig10_Right.txt", skip_footer=(mag_bins+1) * time_bins, usecols=(0,4,5, 11, 10, 12)) # 4,5, for m-bias | 6,7 for c-
data_puyol_compl = np.genfromtxt("data/Fig10_Right.txt", skip_header=3 * (mag_bins+1) * time_bins, usecols=(0, 7, 8, 12, 11, 18)) # 7,8 for m-bias | 13,14 for c-bias

test1 = np.genfromtxt("data/Fig10_Right.txt", skip_footer=(mag_bins+1) * time_bins, usecols=(0,1,2,3))
num_meas = test1[:,0] * test1[:,2] * test1[:,3]

test2 = np.genfromtxt("data/Fig10_Right.txt", skip_header=3 * (mag_bins+1) * time_bins, usecols=(0, 1))
...
fig.savefig("Fig10_Right.pdf", dpi=300, bbox_inches='tight')
