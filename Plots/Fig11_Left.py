...
names_list = np.genfromtxt("data/Fig11_Left.txt", skip_footer=int((mag_bins+1) * run_rm / pujol_every + 3 * (mag_bins+1) * run_lf), dtype="unicode", skip_header=int(3 * (mag_bins+1) * SKIP_FIRST_LF))[:,0]
names_list_2 = np.genfromtxt("data/Fig11_Left.txt", skip_footer=int((mag_bins+1) * run_rm / pujol_every), dtype="unicode", skip_header=int(3 * (mag_bins+1) * SKIP_FIRST_LF + 3 * (mag_bins+1) * run_lf))[:,0]


data_complete = np.genfromtxt("data/Fig11_Left.txt", skip_footer=int((mag_bins+1) * run_rm / pujol_every + 3 * (mag_bins+1) * run_lf),
                              usecols=(0, 4, 5, 8, 9, 1, 2, 3, 10), skip_header=int(3 * (mag_bins+1) * SKIP_FIRST_LF))

data_complete_2 = np.genfromtxt("data/Fig11_Left.txt",
...
data_puyol_compl = np.genfromtxt("data/Fig11_Left.txt", skip_header=int(6 * (mag_bins+1) * run_lf + (mag_bins+1) * SKIP_FIRST_RM),
...
fig.savefig("Fig11_Left.pdf", dpi=300, bbox_inches='tight')
