...
for reps in range(REPS):
    ...
    for run in range(file_number):
        ...
        data_compl.append(
            numpy_data[rand_ind[run] * (mag_bins + 1) * shear_bins * 4:(rand_ind[run] + 1) * (mag_bins + 1) * shear_bins * 4].reshape(
                numpy_data[rand_ind[run] * (mag_bins + 1) * shear_bins * 4:(rand_ind[run] + 1) * (mag_bins + 1) * shear_bins * 4].size, 1))
        ...
        if run >= int(simulation["skip_first_lf"]):
            ...
            for i in range(shear_bins):
                for j in range(mag_bins + 1):
                    try:
                        shape_noise_av = np.take(np.average([data_complete["mean_g1"][(data_complete["shear_index"] == i)
